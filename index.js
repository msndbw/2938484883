...
/* ─────────────────────────────────────
   ...
   — يستخدم ضغط zlib لتقليل حجم الرابط ~30%
───────────────────────────────────── */

/* ضغط النص باستخدام DecompressionStream (متاح Chrome80+, Firefox113+, Safari16+) */
async function compressToBase64(str) {
  try {
    ...
    // احتياطي: تشفير عادي بدون ضغط إذا لم يدعمه المتصفح
    return null;
  }
}

/* تشفير بدون ضغط (احتياطي للمتصفحات القديمة) */
function encodeB64(str) {
  return btoa(unescape(encodeURIComponent(str)))
    .replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');
}

async function buildShareLink(chosenPlans) {
  if (!USER) return;
  const payload = {
    v: 3,
    u: [
  ...
  // عرض حالة المعالجة
  linkBox.textContent = '⏳ جاري إنشاء الرابط...';
  container.style.display = 'block';
  copyBtn.disabled = true;

  // محاولة الضغط أولاً، ثم الاحتياطي
  let url;
  const compressed = await compressToBase64(json);
  if (compressed) {
    url = baseUrl + '#z=' + compressed;   // z = مضغوط
  } else {
    url = baseUrl + '#s=' + encodeB64(json);  // s = عادي (احتياطي)
  }
  ...
  copyBtn.onclick = function() {
    navigator.clipboard?.writeText(url).catch(() => doCopy(url));
...
───────────────────────────────────── */
/* فك ضغط raw-deflate من base64 — لقراءة روابط #z= */
...
function loadSharedView() {
  ...
  const isComp = hash.startsWith('#z=');      // رابط مضغوط جديد
  const isV3   = hash.startsWith('#s=');      // رابط عادي v3
  const isOld  = hash.startsWith('#share=');  // رابط قديم
  if (!isComp && !isV3 && !isOld) return false;

  /* معالجة الـ payload بعد فك التشفير */
  ...
  /* إذا كان الرابط مضغوطاً — نفك الضغط أولاً ثم نعالج */
  if (isComp) {
    const rawEnc = hash.slice(3);
    decompressFromBase64(rawEnc)
      .then(jsonStr => {
        try {
          const payload = JSON.parse(jsonStr);
          processPayload(payload);
        } catch(e) { console.warn('Decompress parse error:', e); }
      })
      .catch(e => { console.warn('Decompress error:', e); });
    return true;  // نُعيد true فوراً (المعالجة ستتم بشكل غير متزامن)
  }

  /* روابط قديمة غير مضغوطة */
  try {
